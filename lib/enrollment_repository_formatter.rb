module EnrollmentFormatter
  ...
  def formatted_hashes_per_enrollment(data)
    kinder = formatted_hashes_per_district(kindergarten_file(data),
    "kindergarten_participation")
    if data[:enrollment][:high_school_graduation].nil?
      formatted = kinder
    else
      grad = formatted_hashes_per_district(graduation_file(data),
  ...
  def formatted_hashes(data)
    group_by_district_name_enrollment(data).values.map do |hash|
      hash.reduce(:merge)
    end
  end
